...
keypad ──pin 4 (keypad side)──► ESP sense ──pin 4 (desk side)──► controller
         (divider → GPIO21)      (GPIO22 → shifter: mirrors keypad wake,
...
| 2 (SWIM) | straight | — |
| 3 (n/c) | straight | — |
| 4 (PIN 20) | **CUT** | keypad side → 4.7k → `GPIO21` node → 10k → GND · desk side ← shifter `HV4`, `LV4` ← `GPIO22` |
| 5 (controller→keypad UART) | straight + tap | tap → shifter `HV2`, `LV2` → `GPIO17` |
| 6 (keypad→controller UART) | **CUT** | keypad side → shifter `HV1`, `LV1` → `GPIO16` · desk side ← shifter `HV3`, `LV3` ← `GPIO23` |
...
- All ESP-side connections live on one header row of a 30-pin devkit (GPIO16/17/21/22/23 + 3V3 + GND) — single-side soldering, and none are boot-strapping pins.
...
Home Assistant entities (via native ESPHome API): a **cover named after the device** (position slider mapped over the calibrated travel range, open/close/stop) and a **`Target Height` number** (cm — type 110, desk goes to 110.0 ± 0.1), both driven by an on-device closed-loop engine (~500 ms wake from sleep, lands within one display quantum, physical keypad presses instantly override any automated move). Plus `Height` (cm, fetched automatically at boot via a wake pulse), buttons `Nudge Up`, `Nudge Down`, `Stop`, `Preset 1/2/3`, an `Emulation Mode` config switch, and diagnostics `Keypad Active` (PIN 20 sense), `Controller Awake` (disabled by default), `Refresh Height`. The node also serves a local web UI + REST API at its IP (`web_server`).
